...
class product_product(models.Model):
    ...
    def _product_available_sql(self):
        ...
        moves_in = self._read_group_by_sql('stock.move', domain_move_in, having=False)
        moves_out = self._read_group_by_sql('stock.move', domain_move_out, having=False)
        quants = self._read_group_by_sql('stock.quant', domain_quant, having=False)
        res = {}
    ...
    def _search_product_quantity(self, domain):
        res = []
        for field, operator, value in domain:
            ...
            if operator == '=':
                operator = '=='
            res.append(('id', 'in', self._search_qty( field, operator, value, context)))

        return res

    def _search_qty(self,  field, operator, value):
        context = context or {}
        domain_move_in, domain_move_out, domain_quant = self._get_product_qyt_domain( [])
        res = {}
        if field == 'incoming_qty':
            res = self._read_group_by_sql( 'stock.move', domain_move_in, having=[(field, operator, value)])
        elif field == 'outgoing_qty':
            res = self._read_group_by_sql( 'stock.move', domain_move_out, having=[(field, operator, value)])
        elif field == 'qty_available':
            res = self._read_group_by_sql( 'stock.quant', domain_quant, having=[(field, operator, value)])
        elif field == 'virtual_available':
            res = self._search_virtual_qty( domain_move_in, domain_move_out, domain_quant, domain_virtual_qty=[(field, operator, value)])
    ...
    def _get_groupby_query(model_name, domain, having=False):
        having = having or []
        model = self.pool.get(model_name)
        query_obj = model._where_calc(domain)
    ...
    def _read_group_by_sql(self, model_name, domain, having=False):
        having = having or []
        query, where_clause_params = self._get_groupby_query( model_name, domain, having=having)
    ...
    def _search_virtual_qty(self, domain_move_in, domain_move_out,
        ...
        query_quant1, where_clause_params_quant1 = self._get_groupby_query( 'stock.quant', domain_quant, having=False)
        query_quant2, where_clause_params_quant2 = self._get_unmoved_product_query(
                     'stock.quant', domain_quant)
        query_in1, where_clause_params_in1 = self._get_groupby_query( 'stock.move', domain_move_in, having=False)
        query_in2, where_clause_params_in2 = self._get_unmoved_product_query(
                     'stock.move', domain_move_in)

        query_out1, where_clause_params_out1 = self._get_groupby_query( 'stock.move', domain_move_out, having=False)
    ...
    def _get_unmoved_product(self, model_name, domain):
        query, where_clause_params = self._get_unmoved_product_query( model_name, domain)
        cr.execute(query, where_clause_params)
        return dict(cr.fetchall())